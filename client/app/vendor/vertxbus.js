...
//
// The issue is that the nicely packaged
// vertx3-eventbus-client available by npm packaging
// is problematic for Webpack when used according to its
// biases (CommonJS packaging).  In the original, Webpack 
// sees the define and tries to load the package as an AMD 
// module.  But the SockJS library provided by sockjs-client
// is a commonjs module.  There are supposed workarounds
// using the Webpack import loader,but I haven't gotten them
// to work nicely.
//
// This "fix" puts the CommonJS first.  I haven't tested
// if the AMD or global sections even work at all.  This should
// be okay since (AFAIK) webpack is the only system that
// transparently handles both CommonJS and AMD, while having
// a strong bias towards CommonJS -- other build systems 
// that only understand AMD will skip past the CommonJS and
// find AMD.
//
// All changes are in the !function(factory)block
//
